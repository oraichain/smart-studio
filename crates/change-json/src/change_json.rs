...
impl From<&Change> for ChangeJson {
    fn from(change: &Change) -> Self {
        ...
        let local_roots = change.roots.as_ref().map(|root| SourceRootJson::from(root, false));
        let library_roots = change.roots.as_ref().map(|roots| SourceRootJson::from(roots, true));
        let files_changed = FilesJson::from(&change.files_changed);
        ChangeJson { crate_graph, local_roots, library_roots, files_changed }
    }
}
        ...
        let result = self
            ...
            .map(|root| {
                ...
                for (file_id, path) in root {
                    ...
                    if let Some(path) =
                        path.as_ref().map(String::to_string).map(VfsPath::new_virtual_path)
                    {
