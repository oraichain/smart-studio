...
impl From<&CrateData> for CrateDataJson {
    fn from(crate_data: &CrateData) -> Self {
        ...
        let display_name = crate_data.display_name.as_ref().map(|name| name.to_string());
...
mod tests {
    ...
    fn serialize_crategraph_check_deps() {
        ...
        let expected_deps = vec![
            DepJson { from: 0, name: "crate2".to_string(), to: 1 },
            DepJson { from: 1, name: "crate3".to_string(), to: 2 },
        ];
