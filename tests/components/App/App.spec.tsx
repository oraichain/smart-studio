...
describe('Tests for App', () => {
  ...
  describe('constructor', () => {
    it('should have the correct initial state', () => {
      ...
      expect(wrapper).toHaveState('workspaceSplits', [{ min: 200, max: 400, value: 200 }, { min: 256 }]);
      expect(wrapper).toHaveState('controlCenterSplits', [{ min: 100 }, { min: 40, value: 256 }]);
    });
  });
  ...
  describe('Toolbar buttons', () => {
    ...
    describe('Gist', () => {
      ...
      it('should export the project to a Gist', async () => {
        ...
        expect(shallow(showToast.mock.calls[1][0])).toContainReact(
          <span>
            "Gist Created!"{' '}
            <a href={'gist-url'} target="_blank" className="toast-span">
              Open in new tab.
            </a>
          </span>
        );
    ...
    describe('Build and Preview', () => {
      it('should build and preview the project when clicking the Build & Preview button', async () => {
        ...
        toolbar
          .find(Button)
          .at(ArcButtonIndex.BuildAndPreview)
          .simulate('click');
        await waitUntil(() => publishArc.mock.calls.length > 0); // Wait until build().then(publishArc) Promise chain resolves
        expect(build).toHaveBeenCalled();
        expect(publishArc).toHaveBeenCalled();
  ...
  describe('ControlCenter', () => {
    it('should handle onToggle', () => {
      ...
      expect(wrapper).toHaveState({
        controlCenterSplits: [{ min: 100 }, { min: 40, value: 40 }]
      });
      onToggle();
      expect(wrapper).toHaveState({
